...
<body>
							...
							<div class="panel-body">
								...
								<div class="form-inline">
									...
									<div class="form-group">
										<label class="sr-only" for="name">企业</label> 
										<input class="form-control" id="name" name="name" placeholder="企业">
									</div>
									
									<div class="form-group">
										<button onclick="loadTab(1)" class="btn btn-danger " type="button"><i class="fa fa-search">&nbsp;搜索</i></button>
										
										<a class="btn btn-primary" href="/master/ocomp/addOComp">增加&nbsp;<i class="fa fa-plus"></i></a>
									</div>
								</div>
	...
	<script type="text/javascript">
		...
		function del(id){
			if(confirm("确认删除吗?")){
				window.location.href="${path}/master/ocomp/delOComp/"+id;
			}
		}
		
		function lock(id){
			if(confirm("确认执行吗?")){
				window.location.href="${path}/master/ocomp/auditOComp/"+id+"/1";
			}
		}
		
		function unlock(id){
			if(confirm("确认执行吗?")){
				window.location.href="${path}/master/ocomp/auditOComp/"+id+"/0";
			}
		}
		
		//加载函数
		function loadTab(s){
			var url="${path}/master/ocomp/ocompList";
			var ps={"s":s};
				ps.p1=$("#name").val();
			...
			$.post(url,ps,function(json){
				if(json.count>0){
					...
					$.each(json.datas,function(i,d){
						html+='<tr>';
						html+='<td>'+(i+1)+'</td>';
						html+='<td>CID：'+(d.id)+'<br>KEY：'+(d.secret)+'</td>';
						html+='<td title="'+(d.eeurl)+'"> <a href="'+(d.eeurl)+'" target="_blank" >'+(d.eename)+'</a></td>';
						html+='<td><img src="'+(d.eelogo)+'" alt="" style="width:40px;height:40px;" /></td>';
						html+='<td>'+(d.resposeUrl)+'</td>';
						...
						html+='<a href="${path}/master/ocomp/editOComp/'+(d.id)+'" class="btn btn-primary  btn-xs"><i class="fa fa-pencil"></i>&nbsp;编辑</a>&nbsp;';
