...
<body>
							...
							<div class="panel-body">
								...
								<div class="form-inline">
									...
									<div class="form-group">
										...
										<a class="btn btn-primary" href="/master/addRole">增加&nbsp;<i class="fa fa-plus"></i></a>
									</div>
								</div>
	...
	<script type="text/javascript">
		...
		function del(id){
			if(confirm("确认删除吗?")){
				window.location.href="${path}/master/delRole/"+id;
			}
		}
		
		function lock(id){
			if(confirm("确认执行吗?")){
				window.location.href="${path}/master/lockRole?id="+id+"&s=1";
			}
		}
		
		function unlock(id){
			if(confirm("确认执行吗?")){
				window.location.href="${path}/master/lockRole?id="+id+"&s=0";
			}
		}
		
		//加载函数
		function loadTab(s){
			var url="${path}/master/roleLists";
			var ps={"s":s};
				ps.r=$("#name").val();
			...
			$.post(url,ps,function(json){
				if(json.count>0){
					...
					$.each(json.datas,function(i,d){
						...
						html+='<a href="${path}/master/uptRole/'+(d.id)+'" class="btn btn-primary  btn-xs"><i class="fa fa-pencil"></i>&nbsp;编辑</a>&nbsp;';
